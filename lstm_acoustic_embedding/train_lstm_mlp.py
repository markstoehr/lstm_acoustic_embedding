...
def train_mlp(options_dict):
    ...
    datasets, word_to_i_map = data_io.load_swbd_labelled_mask(rng, options_dict["data_dir"], options_dict["min_count"])
    train_x, train_m, train_l, train_y = datasets[0]
    dev_x, dev_m, dev_l, dev_y = datasets[1]
    test_x, test_m, test_l, test_y = datasets[2]
    ...
    # Symbolic variables
    i_batch = T.iscalar()   # batch index
    x = T.tensor3("x")       # flattened data of shape (n_data, d_in)
    y = T.ivector("y")      # labels
    m = T.matrix("x")       # flattened data of shape (n_data, d_in)
    l = T.iscalar("y")      # labels

    # Build model
    logger.info("No. of word type targets: " + str(options_dict["d_out"]))
    model = lstm.BatchMultiLayerLSTMMLP(
        rng, x, m, 39, options_dict["d_out"], options_dict["n_hiddens"], options_dict["hidden_layer_specs"], srng, output_type="last", prefix="lstms")
    if options_dict["dropout_rates"] is not None:
    ...
    validate_model = theano.function(
        ...
        outputs=outputs,
        givens={
            m: dev_m[i_batch * options_dict["batch_size"]: (i_batch + 1) * options_dict["batch_size"]].T[:dev_l[i_batch * options_dict["batch_size"]: (i_batch + 1) * options_dict["batch_size"]].max()],
            x: dev_x[i_batch * options_dict["batch_size"]: (i_batch + 1) * options_dict["batch_size"]].swapaxes(0, 1)[:dev_l[i_batch * options_dict["batch_size"]: (i_batch + 1) * options_dict["batch_size"]].max()],
    ...
    test_model = theano.function(
        ...
        outputs=outputs,
        givens={
            m: test_m[i_batch * options_dict["batch_size"]: (i_batch + 1) * options_dict["batch_size"]].T[:test_l[i_batch * options_dict["batch_size"]: (i_batch + 1) * options_dict["batch_size"]].max()],
            x: test_x[i_batch * options_dict["batch_size"]: (i_batch + 1) * options_dict["batch_size"]].swapaxes(0, 1)[:test_l[i_batch * options_dict["batch_size"]: (i_batch + 1) * options_dict["batch_size"]].max()],
    ...
    train_model = theano.function(
        ...
        updates=updates,
        givens={
            m: train_m[i_batch * options_dict["batch_size"]: (i_batch + 1) * options_dict["batch_size"]].T[:train_l[i_batch * options_dict["batch_size"]: (i_batch + 1) * options_dict["batch_size"]].max()],
            x: train_x[i_batch * options_dict["batch_size"]: (i_batch + 1) * options_dict["batch_size"]].swapaxes(0, 1)[:train_l[i_batch * options_dict["batch_size"]: (i_batch + 1) * options_dict["batch_size"]].max()],
